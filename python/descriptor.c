...
static PyUpb_DescriptorBase* PyUpb_DescriptorBase_DoCreate(
  ...
  if (!PyUpb_WeakMap_Add(state->descriptor_map, def, &py_base)) {
...
static PyObject* PyUpb_DescriptorBase_Get(PyUpb_DescriptorType type,
  ...
  PyUpb_ModuleState* state = PyUpb_ModuleState_Get();
  PyObject* ret = PyUpb_WeakMap_Get(state->descriptor_map, def);
  if (ret) return ret;
...
static void PyUpb_DescriptorBase_Dealloc(PyUpb_DescriptorBase* base) {
  ...
  PyUpb_ModuleState* state = PyUpb_ModuleState_MaybeGet();
  if (state && state->descriptor_map) {
    PyUpb_WeakMap_EraseIfEqual(state->descriptor_map, base->def,
                               (PyObject*)base);
  }
...
bool PyUpb_InitDescriptor(PyObject* m) {
  ...
  s->descriptor_map = PyUpb_WeakMap_New();
  if (!s->descriptor_map) return false;
